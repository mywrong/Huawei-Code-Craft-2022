...
//    }
        ...
        unordered_map<int,list<int>> siteSets;
        for(int cidx=0;cidx<this->M;cidx++){
            list<int> l;
            for(int sidx:this->siteSets[cidx]){
                l.push_back(sidx);
        ...
        for (int t = 0; t < this->T; t++) {
            ...
            for (int cidx = 0; cidx < this->M; cidx++) {
                for (int pidx = 0; pidx < stream_nums; pidx++) {
                    ...
                    siteSets[cidx].remove(usedsidx);
                    siteSets[cidx].push_back(usedsidx);
                }
            }
        }
...
//    this->io.output(X, this->dataPath+"/second.txt");
    ...
    sort(siteQ.begin(),siteQ.end(),[&N95Flows](int x,int y){
        return N95Flows[x]>N95Flows[y];
    });
