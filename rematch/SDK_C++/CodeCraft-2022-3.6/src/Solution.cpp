...
void Solution::work() {
    ...
    vector<unordered_map<int,double>> siteCustParams(this->N);
    for(int sidx=0;sidx<this->N;sidx++){
        if(this->custSets[sidx].size()==0)
            continue;
        string sid=this->site_ids[sidx];
        double flow=this->siteDict[sid]/this->custSets[sidx].size();
        for(int cidx:this->custSets[sidx]){
            string cid=this->custom_ids[cidx];
            double sum=0;
            for(int broSidx:this->siteSets[cidx]){
                string broSid=this->site_ids[broSidx];
                sum+=this->siteDict[broSid]/this->custSets[broSidx].size();
            }
            siteCustParams[sidx].emplace(cidx,flow/sum);
        }
    }
    ...
    for (int sidx: siteQ) {
        ...
        for (int t = 0; t < this->T; t++) {
            ...
            int stream_nums = this->stream_ids[time].size();
            for (int cidx: this->custSets[sidx]) {
                int canFlow = int(custTimeNeeds[cidx][t] * siteCustParams[sidx][cidx]);
                for (int pidx = 0; pidx < stream_nums; pidx++) {
                    int need = demandInfo[t][cidx][pidx];
                    if (need > 0 && siteTimeInfo[sidx][t] >= need) {
                        siteTimeInfo[sidx][t] -= need;
