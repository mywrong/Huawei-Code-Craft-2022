...
void Solution::getSiteSets(unordered_map<int, vector<int>> &siteSets) {
    ...
    for (int cidx = 0; cidx < this->M; cidx++) {
        ...
        sort(siteSet.begin(),siteSet.end(),[&](int x,int y){return this->siteDict[this->site_ids[x]]>this->siteDict[this->site_ids[y]];});
...
vector<int> Solution::getSiteQ() {
    ...
    sort(siteQ.begin(), siteQ.end(), [&](int x, int y) { return this->siteDict[this->site_ids[x]] > this->siteDict[this->site_ids[y]]; });
    return siteQ;
