...
//////新鲁棒分配
    ...
    vector<priority_queue<pair<int, int>, vector<pair<int, int>>, decltype(cmp)>> N5Flows(this->N,
                ...
                for (int sidx: this->siteSets[cidx]) {
                    if (siteTimeInfo[sidx][t] >= need) {
                        ...
                        if (tmp > N5Flows[sidx].top().second) {
                            if (N5Ts[sidx].count(t) == 0 ||
                                (N5Ts[sidx].count(t) != 0 && siteTimeUsed1[sidx][t] == oldN95)) {
    ...
    sort(siteQ.begin(), siteQ.end(),[&N95Flows](int x,int y){return N95Flows[x]>N95Flows[y];});
    int migrate_nums = int(this->N * 0.1);
    set<int> migrate_Set;
    vector<int> migrate_list;
    for (int i = 0; i < migrate_nums; i++) {
        migrate_Set.insert(siteQ[i]);
        migrate_list.push_back(siteQ[i]);
    }



    int targetFlow = N95Flows[siteQ[migrate_nums]];

    for (int sidx: migrate_list) {
        string sid = this->site_ids[sidx];
        int T = this->T;
        ...
        for (int j = 0; j < THREAD_WORKERS; j++) {
            Thread.push_back(thread([&, startT, T, n]() {
                ...
                for (int t = sT; t < eT; t++) {
                    string time = this->times[t];
                    int stream_nums = this->stream_ids[time].size();
                    if (siteTimeUsed[sidx][t] > N95Flows[sidx] || siteTimeUsed[sidx][t] <= targetFlow) {
                        continue;
                    }
                    int needCut = siteTimeUsed[sidx][t] - targetFlow;
                    ...
                    for (int cidx = 0; cidx < this->M; cidx++) {
                        ...
                        for (int brotherSidx: brotherSidxs) {
                            if (migrate_Set.count(brotherSidx) != 0)
                                continue;
                            ...
                            tmp = min(this->siteDict[bsid], max(targetFlow, tmp));
...
//    int n = 0;
//    for (int sidx = 0; sidx < this->N; sidx++) {
//        if (siteMaxFlow[sidx] == 0 && this->custSets[sidx].size() > 0)
//            n++;
//    }
//    cout << n << endl;
...
vector<int> Solution::getFullSite(int k) {
    vector<int> siteQ;
    siteQ.reserve(this->N);
    for (int sidx = 0; sidx < this->N; sidx++) {
        siteQ.push_back(sidx);
