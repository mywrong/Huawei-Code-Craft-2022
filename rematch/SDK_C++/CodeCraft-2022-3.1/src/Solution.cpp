...
void Solution::work() {
    ...
    for (int sidx: siteQ) {
        ...
        if(this->siteDict[sid]<this->base_cost*2){
            continue;
        }
    ...
    vector<int> siteFlowSum(this->M,0);
    for(int cidx=0;cidx<this->M;cidx++){
        for(int sidx:this->siteSets[cidx]){
            siteFlowSum[cidx]+=this->siteDict[this->site_ids[sidx]];
        }
    }
    for(int t=0;t<this->T;t++){
        string time=this->times[t];
        int stream_nums=this->stream_ids[time].size();
        for(int cidx=0;cidx<this->M;cidx++){
            int sum=custTimeNeeds[cidx][t];
            int flowSum=siteFlowSum[cidx];
            vector<pair<int,int>> canFlows;
            for(int sidx:this->siteSets[cidx]){
                string sid=this->site_ids[sidx];
                if(this->siteDict[sid]<this->base_cost*2){
                    continue;
                }
                int canFlow=int(sum*this->siteDict[sid]/flowSum);
                sum-=canFlow;
                flowSum-=this->siteDict[sid];
                canFlows.push_back(make_pair(sidx,canFlow));
            }
            for(auto pa:canFlows){
                int sidx=pa.first;
                string sid=this->site_ids[sidx];
                int canFlow=pa.second;
                for(int pidx=0;pidx<stream_nums;pidx++){
                    int need = demandInfo[t][cidx][pidx];
