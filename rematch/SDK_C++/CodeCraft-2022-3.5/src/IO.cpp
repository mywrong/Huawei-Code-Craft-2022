...
void IO::output(vector<vector<vector<int>>> &X,vector<int> &N90Sidxs,string outPath) {
    ...
    for(int i=0;i<10;i++){
        outFile<<this->site_ids[N90Sidxs[i]];
        if(i<9){
            outFile<<",";
        }else{
            outFile<<"\n";
...
void IO::output(vector<vector<vector<int>>> &X,string outPath) {
    IO::exists_test(outPath);
    ofstream outFile;
    outFile.open(outPath);
    if(!outFile.is_open()){
