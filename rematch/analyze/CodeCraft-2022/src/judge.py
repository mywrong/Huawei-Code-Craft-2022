...
class Judge:
    ...
    def getTop10UsedInfo(self):
        k=10
        siteTimeUsed=deepcopy(self.siteTimeUsed)
        siteTimeUsedInfo=[[] for _ in range(self.N)]
        for sidx,sid in enumerate(self.site_id):
            for t in range(self.T):
                siteTimeUsedInfo[sidx].append([t,siteTimeUsed[sid][t]])

        N95Flows=[]
        for sidx,sid in enumerate(self.site_id):
            siteTimeUsedInfo[sidx].sort(key=lambda x:(-x[1],siteTimeUsedInfo[sidx]))
            t=siteTimeUsedInfo[sidx][int(self.T*0.05)][0]
            f=siteTimeUsedInfo[sidx][int(self.T*0.05)][1]
            N95Flows.append([sid,t,f])
        N95Flows.sort(key=lambda x:(-x[2],N95Flows))
        N95Flows=N95Flows[:k]

        usedInfo={}
        for sid,t,f in N95Flows:
            time=self.times[t]
            cidInfo={}
            for cidx,cid in enumerate(self.custom_id):
                if sid in self.ans[time][cid].keys():
                    cidInfo[cid]=self.ans[time][cid][sid]
            cidInfo["95分位"]=f
            cidInfo["time"]=time
            usedInfo[sid]=cidInfo

        for sid,cidInfo in usedInfo.items():
            print("{} ，95分位值 {}，时间 {}".format(sid,cidInfo["95分位"],cidInfo["time"]))
            print("花费的节点及流：")
            for cid,plist in usedInfo[sid].items():
                print("    {}:".format(cid),plist)
