...
class CodeCraft:
    ...
    def work(self):
            ...
            for t in range(self.T):
                ...
                for cidx in self.custSets[sidx]:
                    if siteTimeInfo[sidx][t] > 0:
                        ...
                        s += f
                N95Largest = int(max(N95Largest, s))  # 后N95%时刻可以尽量分配到N95Largest流量
            for t in range(self.T):
                canFlow = N95Largest
                for cidx in self.custSets[sidx]:
        ...
        self.N95Flows = self.get_N95Flows()
        self.siteTimeUsed, self.siteTimeInfo = self.get_siteTimeUsedAndInfo()

        custSetsRevr = [[] for _ in range(self.N)]
        for i in range(self.N):
            for j in range(self.M):
                if self.qosInfo[i][j] < self.qos_constraint:
                    custSetsRevr[i].append(j)
        for i in range(self.N):
            custSetsRevr[i].sort(key=lambda x: (-len(self.siteSets[x]), custSetsRevr[i]))
        self.custSetsRevr=custSetsRevr
        N = min(135, self.N)  # 115最优
        for i in range(N):
            sidx = self.siteQ[i]
            startT = 0
            T = self.T
            n = THREAD_WORKERS
            all_task = []
            # print(self.site_id[sidx],"前:",self.N95Flows[sidx])
            for j in range(THREAD_WORKERS):
                args = [sidx, startT, startT + T // n]
                all_task.append(pool.submit(lambda p: self.reDistribute(*p), args))
                startT += T // n
                T -= T // n
                n -= 1
            wait(all_task, return_when=ALL_COMPLETED)
            tmp = sorted(self.siteTimeUsed[sidx])
            self.N95Flows[sidx] = tmp[self.N95]
            # print(self.site_id[sidx], "后:", self.N95Flows[sidx])
    ...
    def reDistribute(self, sidx, startT, endT):

        N95Flows = self.N95Flows
        siteTimeUsed = self.siteTimeUsed
        dp = self.dp
        for t in range(startT, endT):
            if siteTimeUsed[sidx][t] > N95Flows[sidx]:
                continue
            custQ = self.custSetsRevr[sidx]
            for cidx in custQ:
                ...
                for brotherSidx in brotherSidxs:
                    if brotherSidx != sidx:
                        ...
                        brotherQ.append([brotherSidx, canFlow])

                brotherQ.sort(lambda x: (-x[1], brotherQ))

                for brotherSidx, canFlow in brotherQ:
                    canFlow = min(dp[t][cidx][sidx], canFlow)

                    dp[t][cidx][sidx] -= canFlow
                    siteTimeUsed[sidx][t] -= canFlow

                    dp[t][cidx][brotherSidx] += canFlow
