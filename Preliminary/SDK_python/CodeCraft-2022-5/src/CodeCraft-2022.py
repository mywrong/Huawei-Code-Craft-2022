        ...
        for sidx in self.siteQ:
            ...
            for t in range(self.T):
                if siteTimeInfo[sidx][t]>0:
                    ...
                    for cidx in self.custSets[sidx]:
                        needflow=self.demandInfo[t][cidx]
                        if needflow!=0 and needflow<minflow:
                            minflow=needflow
                            mincidx=cidx
                    if minflow!=MAXINT:
                        f=minflow if siteTimeInfo[sidx][t]>=minflow else siteTimeInfo[sidx][t]
                        siteTimeInfo[sidx][t]-=f
                        sitTimeUsed[sidx][t]+=f
                        self.demandInfo[t][mincidx]-=f
                        out[t][mincidx].append([sidx,f])
