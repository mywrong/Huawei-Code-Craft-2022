            ...
            for t in range(self.T):
                    for cidx in self.custSets[sidx]:
                        if siteTimeInfo[sidx][t] > 0:
                            ...
                            self.demandInfo[t][cidx] -= f
                            out[t][cidx].append([sidx, f])
        N95Flows=[0 for _ in range(self.N)]
        for i in range(self.N):
            tmp=sorted(sitTimeUsed[i])
            N95Flows[i]=tmp[self.N95]
        for t in range(self.T):
                ...
                if needflow!=0:
                    ...
                    for sidx in q:
                        if siteTimeInfo[sidx][t]>0:
                            f=needflow
                            f = f if f <= siteTimeInfo[sidx][t] else siteTimeInfo[sidx][t]
                            if sitTimeUsed[sidx][t]<=N95Flows[sidx]:
                                canFlow=(N95Flows[sidx]-sitTimeUsed[sidx][t])
                                f=f if f<=canFlow else canFlow
                            i=0
