...
class CodeCraft:
    ...
    def work(self):
        ...
        self.canSites = [set() for _ in range(self.T)]

        for t in range(self.T):
            for sidx in range(self.N):
                used = self.siteTimeUsed[sidx][t]
                if used < self.N95Flows[sidx] or (used > self.N95Flows[sidx] and used < self.siteInfo[sidx]):
                    self.canSites[t].add(sidx)
