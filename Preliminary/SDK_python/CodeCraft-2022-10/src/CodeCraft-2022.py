    ...
    def work(self):
        ...
        for sidx in self.siteQ:
            ...
            for t in range(self.T):
                canFlow = N95Largest
                for cidx in self.custSets[sidx]:
                    f = min(N95Largest,demandInfo[t][cidx],siteTimeInfo[sidx][t],canFlow)
                    self.updateInfo(sidx,f,t,cidx,siteTimeInfo,demandInfo)
                    canFlow -= f
        if TEST==2:
            data=[[self.siteInfo[sidx]-siteTimeInfo[sidx][t] for t in range(self.T) ] for sidx in range(self.N)]
        ...
        for t in range(self.T):
            for cidx in range(self.M):
                ...
                if needFlow != 0:
                    ...
                    for sidx in q:
                        f=needFlow
                        if sidx in N5SiteTimes[t]:
                            f=min(f,siteTimeInfo[sidx][t])
                        else:
                            f = min(f, siteTimeInfo[sidx][t],max(0,int(N95Flows[sidx][2]*PARAM[2])-siteTimeUsed[sidx][t]))
                        self.updateInfo(sidx,f,t,cidx,siteTimeInfo,demandInfo)
