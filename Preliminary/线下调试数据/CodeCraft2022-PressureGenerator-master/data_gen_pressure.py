...
def read_input():
    ...
    server_num = ask('input server number (default 135):', 135)
    client_num = ask('input client number (default 35):', 35)
    time_len = ask('input time length (default 8290):', 8290)
...
if __name__ == '__main__':
    ...
    distribute_server()
    if len(sys.argv) == 1:
        output('D:\Doc\Huawei2022\线下调试数据\data5')
    else:
